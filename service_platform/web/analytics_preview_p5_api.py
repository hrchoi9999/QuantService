...
DEFAULT_ANALYTICS_PREVIEW_P5_BUNDLE_DIR = (
    Path(__file__).resolve().parent / "internal_preview" / "analytics_p5"
)
PREVIEW_P5_BUNDLE_FILES = {
    "admin_ops_status": "admin_ops_status_20260325.json",
    "bundle_health": "bundle_health_20260325.json",
}


class AnalyticsPreviewP5LoadError(RuntimeError):
...
@dataclass
class AnalyticsPreviewP5Bundle:
    manifest: dict[str, Any] = field(default_factory=dict)
    admin_ops_status: dict[str, Any] = field(default_factory=dict)
    bundle_health: dict[str, Any] = field(default_factory=dict)
    source_name: str = "analytics-preview-p5-local"
    ...
    def asof(self) -> str | None:
        return (
            ...
            or (self.admin_ops_status.get("meta") or {}).get("asof")
            or (self.bundle_health.get("meta") or {}).get("asof")
        )


class AnalyticsPreviewP5Api:
    def __init__(self, *, root_dir: Path | None = None, cache_ttl_seconds: int = 60) -> None:
        configured_dir = os.getenv("ANALYTICS_PREVIEW_P5_BUNDLE_DIR", "").strip()
        self.root_dir = (
            Path(configured_dir)
            if configured_dir
            else (root_dir or DEFAULT_ANALYTICS_PREVIEW_P5_BUNDLE_DIR)
        )
        self.cache_ttl_seconds = max(cache_ttl_seconds, 0)
        self._lock = Lock()
        self._cached_bundle: AnalyticsPreviewP5Bundle | None = None
        self._cache_expires_at = 0.0

    def load_bundle(self, force_refresh: bool = False) -> AnalyticsPreviewP5Bundle:
        with self._lock:
            now = time.monotonic()
    ...
    def _load_from_directory(self, root_dir: Path) -> AnalyticsPreviewP5Bundle:
        ...
        errors: list[str] = []
        for key, default_name in PREVIEW_P5_BUNDLE_FILES.items():
        ...
        if errors:
            raise AnalyticsPreviewP5LoadError(
                "Preview bundle files are out of sync.",
                errors=errors,
            )

        return AnalyticsPreviewP5Bundle(
            manifest=manifest,
            admin_ops_status=payloads["admin_ops_status"],
            bundle_health=payloads["bundle_health"],
        )
